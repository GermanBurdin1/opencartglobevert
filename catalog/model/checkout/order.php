...
class Order extends \Opencart\System\Engine\Model {
	...
	public function addOrder(array $data): int {
		$this->log->write('Data in addOrder: ' . print_r($data, true));
		$defaults = [
			'invoice_prefix' => '',
			'store_id' => 0,
			'store_name' => '',
			...
			'payment_custom_field' => json_encode([]),
			'payment_method' => '',
			...
			'shipping_custom_field' => json_encode([]),
			'shipping_method' => '',
			'comment' => '',
			'total' => 0.0,
			'affiliate_id' => 0,
		...
		// Объединяем массив данных с значениями по умолчанию
